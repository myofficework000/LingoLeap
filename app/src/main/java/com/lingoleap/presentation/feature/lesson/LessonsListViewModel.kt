...
sealed interface LessonsListEvent : UiEvent {
    data object Back : LessonsListEvent
    data class LessonClicked(val lessonId: String) : LessonsListEvent
}
@HiltViewModel
class LessonsListViewModel @Inject constructor(
    private val getCoursesUseCase: GetCoursesUseCase,
    private val getLearnerProgressUseCase:
    ...
    private fun loadLessons() {
        viewModelScope.launch {
            try {
                ...
                val activeCourse = courses.firstOrNull { it.id == progress.activeCourseId }

                _state.value = LessonsListState(course = activeCourse, progress = progress, isLoading = false)

            } catch (exception: Exception) {

                _state.value =
                    LessonsListState(isLoading = false,
                        error = exception.message ?: "Unable to load lessons"
                    )
            }
        }
    }
